...
# Thread for chatbot processing
def process_chatbot_response():
    global sign_string, last_sign_time
    while True:
        # Check if sign_string has accumulated and is ready for chatbot interaction
        ...
        time.sleep(1)  # Small delay to avoid too tight a loop

# Run the process_chatbot_response function in a separate thread
import threading
threading.Thread(target=process_chatbot_response, daemon=True).start()
